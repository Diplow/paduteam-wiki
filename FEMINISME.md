---
name: FÉMINISME — suivi d'ingestion
...
**Critère de sélection initial** : titres mentionnant féminisme, VSS, viol, misogynie, mascus, incels, Némésis, Thaïs d'Escufon, Mazan, Quatennens, transphobie, pédocriminalité, #metoo, féminicides — ou co-animation explicite Dr Zoé sur sujet genre.

**Déjà ingéré** (hors périmètre des batches ci-dessous) :
- `Le GRAPHIQUE est-il vraiment SEXISTE` (débat interne PR)
- `2 MÂLES TOXIQUES MARXPLIQUENT le GRAPHIQUE`
- `Les INCELS ont ENFIN leur SAINT GRAPHIQUE`
- `KROC BLANC 100 000€ D'AMENDE ET 7 ANS DE PRISON` (affaire pédoporno)
- `PAYE TON DROITARD #7.1 PADU L'ADO CONFUS`
- `AFFAIRE DANY` (ZawaProd)
- `TODD DÉSASTREUX SUR LA NATALITÉ — IL ACCUSE LE FÉMINISME`
- `FROM TROTSKISTE TO RACISTE`

**Doublons EN à exclure** :
- `VALEK ACCUSES ZOÉ OF BEING AN ACCOMPLICE TO PEDOCRIMINALS` (EN — à vérifier si équivalent FR existe)
...
## Vidéos de second rang (à évaluer au cas par cas)

Transcripts avec un angle féministe/VSS mais dont le sujet central est ailleurs, ou qui forment des unités isolées difficiles à batcher :

- `L'AVIS SCANDALEUX DE CNEWS SUR L'AVORTEMENT` — IVG, unité isolée, à traiter individuellement ou dans un futur batch "droits reproductifs"
- `LOUIS SAN CATASTROPHIQUE sur le FÉMINISME au JAPON` (x Dr Zoé) — pourrait rejoindre un batch "féminisme comme grille d'analyse"
- `ONLYFAN — BAISER avec 100 MECS en 1 JOUR est-il VRAIMENT MARXISTE` (x Dr Zoé) — débat théorique féminisme/marxisme
- `CUISINE et POLITIQUE — nos AVIS PROBLÉMATIQUES` (x Dr Zoé) — contenu plus léger, charge mentale
- `CE DROITARD VEUT QUE LES ADOLESCENTES S'HABILLENT SXY` (x Dr Zoé) — pourrait rejoindre Batch B ou C
- `THAÏS et VALEK nous APPRENNENT à BAISER` — pourrait rejoindre Batch B ou C (transversal)
- `DR ZOÉ apprend à BAISER à ce PUCIX` — pourrait rejoindre Batch B
- `AUJOURD'HUI TON VIOLEUR, DEMAIN TON DOCTEUR #metoomedecine` (x Dr Zoé) — spécifique corpo médicale, pourrait rejoindre un batch santé/sécu
- `La POLICE de RETAILLEAU MALTRAITE une JEUNE FEMME suite au 10 SEPTEMBRE` — plus répression que féminisme stricto sensu
- `HARCELÉ par la MEUTE LFI — LYES LOUFFOK TRAITRE ou VICTIME` — tangent, plutôt gauche/répression
- `VALEK ACCUSES ZOÉ OF BEING AN ACCOMPLICE TO PEDOCRIMINALS` (EN) — vérifier s'il existe une version FR à ingérer

---

## Notes et décisions

- **Ordre recommandé** : A → C → B → D → E → G → F
  - A pose le socle analytique (VSS comme fait social, pas monstruosité individuelle)
  - C documente l'ennemi structurant côté droite (Némésis, Thaïs) et révèle la stratégie de détournement du féminisme
  - B montre l'écosystème mascu/incel comme industrie — les coachs en séduction comme pendant masculin de Némésis
  - D applique la grille aux VSS célèbres (show-business, pouvoir médiatique)
  - E gère l'autocritique interne (front délicat — Quatennens, Prévost, Tondelier/PR)
  - G traite la pédocriminalité comme ligne de fracture de classe
  - F (transphobie) peut se faire en parallèle — relativement autonome des autres batches
- **Enjeu structurant à créer en priorité** (batch A) : fiche enjeu « Féminisme matérialiste » ou « VSS comme fait social » si elle n'existe pas déjà
- **Entités-socles probables** : Dr Zoé (évidemment), Thaïs d'Escufon, Alice Cordier, Collectif Némésis, Gérard Depardieu, Arthur, Adrien Quatennens, Killian Senseï, Valek, Ary Abittan, NousToutes
- **Concepts structurants** : masculinisme, VSS, soumission chimique, féminicide, féminisme identitaire (vs matérialiste), transphobie, pédocriminalité bourgeoise, charge mentale
- **Taille des batches** : A=4, B=11, C=11, D=10, E=3, F=4, G=3. B et C sont en limite haute (11) — envisager scission si trop lourd au moment de l'ingestion.
- **Point d'attention batch E** (vérifié sur transcripts) :
  - *Quatennens* : la critique est **bien réelle et virulente**. Zoé qualifie l'interview BFM de Quatennens de « gravissime », parle de « red flag », démonte sa stratégie de défense (« j'ai fauté une fois »), et juge la défense de Mélenchon « catastrophique ». Le mot « erreur » dans le titre n'est pas un euphémisme clickbait — c'est la position assumée. Vraie autocritique de la gauche.
  - *Tondelier / Positions Revue* : exactement l'inverse, le titre est un **leurre clickbait complet**. Zoé n'accorde « pas le point » de la misogynie dès la première minute et passe le reste de la vidéo à **défendre** Positions Revue, à expliquer pourquoi « Karen » est un terme légitime appliqué à Tondelier (féminisme noir critiquant le féminisme blanc bourgeois), et à attaquer les Verts pour leur féminisme essentialisant. Pas de conflit interne — c'est une vidéo *contre* les accusateurs de PR.
