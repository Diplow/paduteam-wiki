---
type: organisation
domaine: [politique-intérieure]
thèmes: [Palestine]
aliases: [TF1]
date created: Monday, April 13th 2026, 12:00:00 pm
date modified: Monday, April 13th 2026, 12:00:00 pm
skill_version: ingest-batch-2026-04-13
---
#domaine/politique-intérieure #thème/Palestine
# TF1

## Position dans l'analyse PaduTeam
Première chaîne de télévision française (groupe Bouygues). Pour la PaduTeam, [[Chiens de garde]] emblématique du dispositif médiatique bourgeois. Dans le contexte du batch Gaza, TF1 est analysée comme **complice active du génocide** pour avoir platformé [[Benjamin Netanyahu]] en prime time (20h, heure de grande écoute) alors qu'il est sous mandat d'arrêt de la [[Cour Penale Internationale]] pour crimes contre l'humanité.

## Dynamique
Chris ([[NETANYAHOU EN ROUE LIBRE SUR TF1LCI]]) : « Imaginez si on recevait Poutine à 20h sur TF1 — évidemment, ça serait inadmissible. Sauf qu'on ne reçoit pas Poutine. Sauf que pour Netanyahu, ça paraît un peu normal. » Le traitement différentiel réservé à Netanyahu par rapport à d'autres dirigeants contestés révèle l'alignement atlantiste-sioniste de la chaîne. L'interview est menée par [[Darius Rochebin]] sur [[LCI]] (filiale câblée du groupe).

**Entre le direct et la rediffusion**, l'interview sur TF1/LCI offre à Netanyahu :
- un platform de masse (millions de spectateurs),
- une légitimation médiatique (présentateur « neutre » le traitant comme dirigeant ordinaire),
- un temps de parole que ses accusateurs n'obtiennent jamais,
- une rediffusion et des extraits sur réseaux sociaux.

**103+ journalistes tués.** Chris insiste : « 103 journalistes morts à Gaza, Netanyahu dans un fauteuil au 20h... c'est incroyable que des journalistes en France se permettent de faire ça. » La complicité corporative (journalistes français interviewant celui qui a fait tuer leurs confrères palestiniens) est pour PaduTeam un scandale professionnel majeur.

## Figures clés
[[Darius Rochebin]] (journaliste LCI/TF1, figure de l'interview).

## Vidéos où TF1 est analysée
- [[NETANYAHOU EN ROUE LIBRE SUR TF1LCI]] — Cas central : platformage de Netanyahu
