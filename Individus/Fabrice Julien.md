---
type: individu
domaine: [société]
thèmes: [masculinisme, VSS]
aliases: [Le Coach en Séduction, Le Coach en Seduction]
skill_version: "1.0"
date created: 2026-04-13
---
#domaine/société #thème/masculinisme #thème/VSS

# Fabrice Julien

## Profil synthétique

Créateur YouTube se présentant comme "Le Coach en Séduction le plus flippant", spécialisé dans la street pickup — aborder des femmes dans la rue. Analyse PaduTeam : ses techniques ne sont pas du conseil amoureux mais de la prédation codifiée avec sélection des femmes vulnérables. Le plus "flippant" est son propre qualificatif, assumé.

## Stratégie et trajectoire

Fabrice Julien enseigne des techniques d'approche dans la rue avec une codification précise : identifier les femmes en état émotionnel vulnérable (choc récent, solitude), utiliser des prétextes pour les isoler (technique dite "des toilettes" — prétendre vouloir trouver des toilettes pour les entraîner dans un espace isolé), maintenir la pression jusqu'à l'accord.

La PaduTeam note que ce qui est enseigné n'est pas de la séduction — c'est un protocole de contournement du consentement déguisé en conseil. Les vidéos sont suffisamment explicites pour que les commentaires révèlent l'intention : des commentateurs expriment vouloir "revenir à la Préhistoire" pour pouvoir violer sans conséquences. Ce ne sont pas des dérapages isolés — ils reflètent la logique implicite de l'enseignement.

La spécificité de Fabrice Julien dans l'écosystème mascu-coaching : il est moins idéologue que les autres (pas de grandes théories sur les alphas, le complotisme ou le féminisme) — il est plus directement opérationnel dans la codification de la prédation physique.

## Relations
- [[Dr Zoe]] — analyste dans la vidéo Le Coach en Séduction
- [[Killian Sensei]], [[Valek]], [[Alex Hitchens]] — pairs dans l'[[Industrie de la seduction]]

## Concepts associés
[[Culture du viol]], [[Industrie de la seduction]], [[Masculinisme individualiste]]

## Vidéos où Fabrice Julien est analysé
- [[LE COACH SEDUCTION FLIPPANT]] — analyse centrale de ses techniques
